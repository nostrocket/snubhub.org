...
<div
  ...
  title={npub}
  class:cursor-pointer={withLink}
  class:hover:underline={withLink}
  on:click={() => {
    if (withLink) {
      window.open('https://nosta.me/' + pubkey)
    }
  }}
>
  {#if withPicture}
  ...
  {/if}
  <div class="text-ellipsis overflow-hidden text-rose-700 pl-1" title={npub}>
    {name}
  </div>
</div>
